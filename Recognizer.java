...
public class Recognizer {
    ...
    private void insertOnePoint()
    {
        ...
        {   if ( (int) userStroke [i].distance (userStroke [1+i]) > maxDistance)
           {   maxDistance = (int) userStroke [i].distance (userStroke [1+i]);
               maxPosition = i;
           }
        }
     ...
     *            the score
     */
    public double computeScore (int IndexDigitToCompare)
    {   double score =0;
        for (int i=0; i<nextFree; i++)
        {   score+= userStroke[i].distance (baseSet [IndexDigitToCompare] [i] );
        }
        return score;
    }
